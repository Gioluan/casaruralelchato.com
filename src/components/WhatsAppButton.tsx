...
export default function WhatsAppButton({ locale }: { locale: Locale }) {
  ...
  return (
    ...
    >
      {showTooltip && (
        <div className="hidden md:block bg-forest-deep text-cream px-5 py-3 text-sm shadow-2xl max-w-xs animate-[rise_0.5s_ease-out] relative">
          {t.tooltip}
          <span className="absolute top-1/2 -right-2 -translate-y-1/2 w-0 h-0 border-t-[8px] border-t-transparent border-b-[8px] border-b-transparent border-l-[8px] border-l-forest-deep" />
        </div>
      )}
      <a
        href={href}
        ...
        className="flex items-center justify-center w-14 h-14 md:w-16 md:h-16 rounded-full bg-[#25D366] text-white shadow-[0_10px_30px_-8px_rgba(37,211,102,0.55)] ring-4 ring-cream hover:scale-105 hover:shadow-[0_14px_40px_-8px_rgba(37,211,102,0.7)] transition-all duration-300"
      >
        <svg
          viewBox="0 0 32 32"
